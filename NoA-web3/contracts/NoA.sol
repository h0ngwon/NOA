...
contract NoA is ERC721, ERC721Enumerable {

    event PhotoUploaded (uint256 indexed tokenId, bytes photo, string title, string description, uint256 timestamp);
   ...
   */
    function uploadPhoto(bytes memory photo, string memory title, string memory description) public {
        ...
        emit PhotoUploaded(tokenId, photo, title, description, now);
    }
